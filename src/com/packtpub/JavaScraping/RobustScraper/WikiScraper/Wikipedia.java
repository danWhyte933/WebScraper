...
public class Wikipedia {
    /**
     * Crawls both English and German Wikipedia. Finds 10 random German Wikipedias by following redirects * through the "special:random" page, and returns the country of origin for the first IP address * (if any) that it finds on the page. After this, does the same for English Wikipedia edits. * @param args
    ...
    private static String getRandomHist(String language) {
        Webpage randomPage = new Webpage("http://" + language + ".wikipedia.org/wiki/Special:Ran dom");
        String randomUrl = "";
        try {
            if (language == "en") {
    ...
    private static String getEditCountry(String histPageUrl) {
        ...
        for (Element row : rows) {
            ...
            if (row.select(".mw-userlink").text().matches("^(?:[0-9]{1,3}.){3}[0-9]{1,3}$")) { //"Username" is an IP Address IPResolver IPAddress = new IPResolver(row.select(".mw-userlink").text()); return IPAddress.getCountry(); } }
