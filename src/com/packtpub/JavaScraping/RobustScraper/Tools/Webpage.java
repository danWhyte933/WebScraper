...
public String url;
    public URL urlObj;
    /** * Sets urlObj, throws error if the URL is invalid * @param declaredUrl */

    public Webpage(String declaredUrl){ url = declaredUrl; urlObj = null;

    try{ urlObj = new URL(url); }
    catch(MalformedURLException e){ System.out.println("The url was malformed!"); } }
    ...
    public String getRedirect() throws MalformedURLException, IOException{ try{ return getRedirect(false);

    }catch(MalformedURLException e){ System.out.println("The url was malformed!"); }
    catch(IOException e){ System.out.println("There was an error connecting to the website"); }
     ...
     * * @return URL that the page redirects to * @throws MalformedURLException
     * * @throws IOException */

    public String getRedirect(Boolean secondRedirect) throws MalformedURLException,
            IOException{ HttpURLConnection URLCon = (HttpURLConnection)urlObj.openConnection();
...
//If a second redirect is required if(secondRedirect){ try{ URL newURL = new URL(header); HttpURLConnection newCon = (HttpURLConnection)newURL.openConnection();
...
/** * Gets the text version of the webpage * @return */
